...
public class DuckController : MonoBehaviour
{
	...
	public AudioClip bloop, duckHit;
    public float duckSpeed = 5.0f;
    public float flapSpeed = 5.0f;

    enum DuckState { SurfaceSwimming, Swimming, Flying };
    ...
    void Start()
    {
        ...
        duckState = DuckState.SurfaceSwimming;
        isInvul = false;
    }
    ...
    void Update()
    {
        ...
        if (Input.GetAxis("Horizontal1") > 0f)
        {
            sr.flipX = false;
        }
        else if (Input.GetAxis("Horizontal1") < 0f)
        {
            sr.flipX = true;
        }
